...
function countVowels(str) {
  const a = str.split('a').length - 1;
  const e = str.split('e').length - 1;
  const i = str.split('i').length - 1;
  const o = str.split('o').length - 1;
  const u = str.split('u').length - 1;
  ...
  const U = str.split('U').length - 1;
  const Y = str.split('Y').length - 1;
  const sum = a + e + i + o + u + y + A + E + I + O + U + Y;
  return sum;
}
...
function findLongestWord(sentence) {
  ...
  const len = arraySentence.length;
  let n = 0;
  for (let i = 0; i < len; i += 1) {
    if (arraySentence[i].length > arraySentence[n].length) {
      n = i;
    }
  }
...
function extractNameFromTemplate(value) {
  const b = value.length - 1;
  const a = value.slice(7, b);
  return a;
}
...
function unbracketTag(str) {
  const b = str.length - 1;
  const a = str.slice(1, b);
  return a;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (n >= 65 && n <= 90) {
      result += String.fromCharCode(((n - 65 + 13) % 26) + 65);
    } else if (n >= 97 && n <= 122) {
      result += String.fromCharCode(((n - 97 + 13) % 26) + 97);
    } else {
      result += str[i];
    }
  }
